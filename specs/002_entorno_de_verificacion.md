...
| 2026-09-15 | Reafirmada como propuesta para el siguiente ciclo por el informe de cierre del 004/005; sin cambios de contenido | Tres hallazgos del ciclo la piden: la causa exacta del 403 del lider-tienda se midio pero no pudo demostrarse (T1 de la 005, dos causas con el mismo codigo); el indice ausente de `walletEntries` para `sum(amountCop)` solo aparecio al llegar a produccion (T14, defecto latente de la 001); y la regla reescrita en T2 no pudo ni validarse de sintaxis en local, asi que el 403 siguio vivo en produccion hasta la firma humana de T13. Las reglas de Firestore siguen sin ser probables en unidad |
