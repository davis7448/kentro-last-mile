...
- [x] **T49: El alta de un lider no deja comunidades a medias**
  * Requisitos cubiertos: RF_55
  * Archivos: `functions/src/community-leader-create.ts` (nuevo), `functions/src/communities.ts`, `src/lib/community-signup.test.ts`. Modulo aparte del registro publico a proposito: comparten la idea, no las reglas — aquel es sin sesion (RF_43/RF_45/RF_10), este es alta administrativa.
  * **El fallo:** `createCommunityLeader` (`communities.ts`) escribe la comunidad y **reserva el enlace** en una transaccion, y **despues** crea la cuenta de acceso. **No hay reversion.** Si el correo ya existe —el caso mas probable, y el que motivo esta tarea— el segundo paso falla con lo primero ya escrito: queda una **comunidad fantasma con su enlace reservado**. Y el reintento con el mismo nombre corto responde "ese nombre corto ya esta en uso", apuntando a la que se acaba de crear sin querer.
  * Hay un **segundo punto de fallo** que es peor: si `setCustomUserClaims` falla despues de crear la cuenta, queda una cuenta que **entra y no tiene rol**.
  * Accion: planificador puro de reversion, al estilo del `signupRollbackPlan` que ya existe para el registro publico (RF_45) — pero **con el orden invertido**, porque aqui Firestore va primero. Mas la comprobacion previa del correo, que resuelve limpiamente el caso comun sin escribir nada. **Las dos cosas, no una:** la comprobacion previa tiene carrera (dos administradores a la vez), asi que no sustituye a la reversion.
  * Verificacion: correo repetido → no queda ni comunidad ni enlace, y el nombre corto se puede reutilizar de inmediato. Fallo al fijar el rol → no queda ni la cuenta. Exito → no se revierte nada.
